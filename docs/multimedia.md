...
!!! Abstract "课程信息"
    ...
    **Grading Policy**：20 % 课堂参与 + 40 % 课程项目（10 % 课堂展示，名额有限需要报名）+ 40 % 期末测试 + 5 % Bonus（论文阅读）

    >   半学期的课，但是课程密度就我个人而言是比较高的水平，一周有两天早八。课程期间会有随机课堂小测当做点名。
...
2.  8-bit Grey-level Image (8位灰度图像)
    ...
    每个像素用一个字节存储，对应 0 到 255 的灰度值。灰度值为 0 时，该像素为纯黑色，为 255 时，该像素为纯白色。也就是说，n-bit 的灰度代表每个像素需要 n-bit 编码，对应 $2^n$ 级灰度。

    -   **存储结构**：整张图像可以看作是一个二维的像素值数组 (bitmap)。也可以被视作由 8 个 1-bit 的**位平面 (bitplanes)** 叠加而成。

    ??? Question "黑白打印机为什么可以打出灰度图像？- 抖动与抖动算法"

        黑白打印机其实还是一个二元 (1-bit) 的设备。我们能够看到灰度变化，事实上是打印机在用空间分辨率 (spatial resolution) 代替灰度分辨率 (intensity resolution)，即将原图的一个像素细分再填入黑白点。这个过程通常被称为**抖动 (Dithering)**。

        一个 $N \times N$ 的矩阵最多可以表示 $N^2 + 1$ 个灰度等级。反过来说，如果我们想要 25 级灰度图，那么打印机需要在一个像素面积内至少要打 24 个点。
        
        衡量打印机性能的一个重要指标是 DPI，即在单位长度 (1 英寸) 内可以打出多少个点。

        ??? Example ""
        
        
        对于中间等级的灰度表示，常有两种处理策略：
        
        1.   **有序抖动 (Ordered Dither)**：为了避免图像被放大，可以使用一个“标准模式矩阵 (Standard Pattern Matrix)”。将灰度图矩阵与该标准矩阵通过取模运算进行对齐比较（如 $i = x \pmod n,\ j = y \pmod n$），若原图灰度值大于矩阵中的设定值，则在该位置打印黑点，否则不打印。
...
需要清楚每一类图像的<u>定义、存储空间的算法、应用的方式</u>。

### 8 位彩色图像

8 位的彩色图像是一种很特殊的图像。我们需要清楚从 24 位真彩色图像如何转换到 8 位彩色图像。
