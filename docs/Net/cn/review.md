...
*   **纠错与检错能力（重点公式）：**
    ...
    *   *Slide 42 例子:* 要纠正2个错误，需要海明距离为5的编码。如果只有距离3，无法纠正2个错误。
...
*   **循环冗余校验 (CRC):**
    ...
    *   *Slide 45 给出了具体的计算图示，考试中常考具体的CRC计算。*
...
*   *Slide 49 的计算题:* 这种题目要注意**有效利用率**。如果包含头部开销和ACK发送时间，计算公式需调整：
    *   有效数据发送时间 = (Payload size) / Rate
    *   总周期 = Data Trans + Prop + ACK Trans + Prop (假设处理时间忽略)
    *   有效利用率 = 有效数据发送时间 / 总周期。
...
*   *例如*，对于SR协议，如果序列号是3 bit (0-7)，发送窗口最大只能是4，否则接收方无法区分是重传的旧帧还是新的帧。
