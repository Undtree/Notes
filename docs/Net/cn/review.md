...
-   **被动攻击**
    -   定义：攻击者只是观察和分析网络中的数据流，不干扰信息的传输。
    -   类型：
        1.  窃听 (Eavesdropping/Sniffing)：截获带有敏感信息（如密码）的数据包。
        2.  流量分析 (Traffic Analysis)：即使内容加密，通过分析通信的频率、长度、源/目地址来推断信息。
    -   特点：很难被检测出来，重点在于预防（如使用加密）。
-   **主动攻击**
    -   定义：攻击者对数据流进行各种处理，会对网络产生影响。
    -   类型：
        1.  篡改 (Modification)：修改报文内容。
        2.  伪造 (Fabrication)：伪装成别人发送报文。
        3.  中断 (Interruption)：切断通信链路。
        4.  恶意程序。
        5.  拒绝服务 (DoS/DDoS)：
            -   DoS：向服务器发送大量请求，耗尽其资源（CPU或带宽），使其无法服务正常用户。
            -   DDoS (分布式)：控制大量“僵尸主机 (Zombies/Botnet)”同时发起攻击，更难防御。
...
密码学是网络安全的基础。在其中，密钥系统更是密码学的重中之重。密钥系统的安全性完全依赖于对密钥的安全保护，算法通常是公开的。

#### 1. 对称密钥密码体制

加密密钥 ($K_A$) 和解密密钥 ($K_B$) 是相同的，或由一个可以推出另一个。

*   **公式**：$Y = E_K(X)$ （加密），$X = D_K(Y)$ （解密）。
*   **常见算法**：
    *   **DES (Data Encryption Standard)**：
        *   1977年标准，**56位**密钥。
        *   **缺点**：密钥太短，现代计算机极易通过暴力破解（穷举）攻破。已不再安全。
    *   **3DES (Triple DES)**：
        *   使用3个密钥执行三次DES算法。解决了密钥长度问题，作为过渡方案。
    *   **AES (Advanced Encryption Standard)**：
        *   **现行标准**。支持 128、192、256 位密钥。
        *   效率高（硬件软件实现都快），安全性高（破解需要万亿年）。

#### 2. 公钥密码体制
*   **原理**：使用不同的密钥进行加密和解密。由 Diffie 和 Hellman 提出，**RSA** 是最著名的实现。
*   **密钥对**：
    *   **公钥 (PK, Public Key)**：向全网公开。
    *   **私钥 (SK, Secret Key)**：用户自己保密。
*   **特性**：
    *   用公钥加密，必须用对应的私钥解密：$D_{SK}(E_{PK}(X)) = X$。
    *   用私钥加密，必须用对应的公钥解密（用于签名）：$D_{PK}(E_{SK}(X)) = X$。
    *   **不能**通过公钥推导出私钥。
*   **优势**：解决了对称加密中**密钥分发**的难题（不需要通过信使传输密钥）。

### 完整性、鉴别与密钥分发

#### 1. 数字签名 (Digital Signature)
*   **目的**：证明报文的真实来源，防止伪造和抵赖。
*   **要求**：接收方能核实、任何人不能伪造、发送方不能抵赖。
*   **实现流程**：
    1.  发送方用自己的**私钥 (SK)** 对报文（或报文摘要）进行加密。
    2.  接收方用发送方的**公钥 (PK)** 进行解密。
    3.  如果能解密出正确内容，证明该报文确实是发送方发的（因为只有他有私钥）。
*   **效率优化**：由于公钥算法运算慢，通常是对报文的**摘要 (Digest/Hash)** 进行签名，而不是对整个长报文签名。

#### 2. 实体鉴别 (Entity Authentication)
*   **目的**：验证通信对方的身份（是真李逵还是李鬼）。
*   **重放攻击 (Replay Attack)**：攻击者截获了A发给B的加密口令，稍后重新发给B，试图伪装成A。
*   **解决方案 —— 不重数 (Nonce)**：
    *   **机制**：B向A发送一个**随机数 (Nonce)**（每次会话都不同）。
    *   A必须用共享密钥加密这个Nonce并发回给B。
    *   B解密后验证数字是否正确。
    *   **原理**：攻击者截获旧的加密报文也没用，因为Nonce变了，旧报文解密后不对。

#### 3. 密钥分发 (Key Distribution)
*   **对称密钥分发**：
    *   需要**KDC (密钥分配中心)**。KDC 是可信第三方，负责生成会话密钥并分发给通信双方。
*   **公钥分发**：
    *   **问题**：如何确保你拿到的“A的公钥”真的是A的，而不是攻击者伪造的？
    *   **解决方案**：**CA (认证中心)**。
    *   **数字证书**：CA用自己的**私钥**对“A的身份信息+A的公钥”进行签名，生成证书。用户拿到证书后，用CA的公钥验证，确保证书未被篡改且属于A。

### 网络安全协议与设施

#### 1. 网络层安全：IPsec
*   **定位**：在 IP 层提供安全（加密和鉴别），对上层透明。
*   **两种工作方式**：
    *   **运输方式 (Transport Mode)**：只加密 IP 数据报的数据部分（载荷），IP 首部不加密。用于端到端通信。
    *   **隧道方式 (Tunnel Mode)**：将整个 IP 数据报（含首部）加密，封装在一个新的 IP 数据报中。主要用于 **VPN (虚拟专用网)**。
*   **重要细节**：无论哪种方式，最外层的 IP 首部都是**不加密**的（否则路由器无法转发）。

#### 2. 运输层安全：SSL / TLS
*   **定位**：介于应用层和运输层之间。
*   **应用**：**HTTPS** = HTTP + SSL/TLS。
*   **功能**：在 TCP 连接之上建立安全通道，提供加密、数据完整性和端点鉴别。

#### 3. 应用层安全
*   **PGP (Pretty Good Privacy)**：用于电子邮件加密。

#### 4. 防火墙 (Firewall)
*   **定义**：一种访问控制技术，置于不同网络安全域之间（如内网和外网之间）。
*   **技术分类**：
    *   **分组过滤 (Packet Filtering)**：在网络层/运输层工作。根据 IP 地址、端口号、协议类型（TCP/UDP）过滤数据包。
        *   *缺点*：无法检查应用层数据（如是否包含病毒）。
    *   **应用网关 (Application Gateway)**：在应用层工作（代理服务器）。检查应用层数据内容。

#### 5. 入侵检测系统 (IDS)
*   **作用**：防火墙无法阻止所有攻击（如内部攻击或通过允许端口的恶意流量）。IDS 用于实时监控网络流量，检测异常行为并报警。
