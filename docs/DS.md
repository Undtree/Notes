...
* 优先队列是一种特殊的数据结构，它的主要特点是每次总是**删除具有最高（或最低）优先级的元素** 。
...
* 1. 结构性质 (Structure Property)：二叉堆在逻辑上必须是一棵**完全二叉树 (Complete Binary Tree)** 。这意味着除了最底层外，其余各层都被完全填满，且最底层节点从左向右紧密排列 。高度为 $h$ 的树其节点总数在 $2^h$ 到 $2^{h+1}-1$ 之间，树的高度为 $h = \lfloor \log N \rfloor$ 。
