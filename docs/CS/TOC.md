...
*   此类语言称为 **递归可枚举语言 (Recursively Enumerable, R.E.)，或者等价地被称作图灵可枚举语言**。不难发现，如果一个语言是递归的，它一定是 R.E. 

!!! Tip "这也就是说，相较于只有接受或拒绝两个状态的 PDA 或者 FA，图灵机即使只有 2 个停机状态（$y,n$），也可以通过永不停机来给出一个答案。"

!!! Note "R.E. 语言的性质"
    
    课件（或者是课本）在接下来的一章有一个分章节来讲这个。我想作为笔记我就放在这里说了，如有问题请随时提出！

    $L$ 是递归的，当且仅当 $L$ 和 $\bar{L}$ 都是 R.E.。证明：

    -   $L$ 是递归的，那么它一定是 R.E.
    -   $L$ 是递归的，那么 $\bar{L}$ 一定也是递归的（很显然，我们把判定 $L$ 的图灵机输出对调就可以了），所以 $\bar{L}$ 也是 R.E.
    -   假如 $L$ 和 $\bar{L}$ 都是 R.E. 的：
        -   对于给定的字符串 $w$，要么它属于 $L$，要么它属于 $\bar{L}$。所以这两个语言的半判定器是必然停机的，也就能判定 $L$ 了。
   
??? Question "图灵可枚举又是什么？"

    -   **定义**：我们称一个图灵机 $M$ 可以枚举语言 $L$，当且仅当 $M$ 中的某个固定的状态 $q$ 满足：
    
        $$
            L = \{w: (s, \triangleright \underline{\sqcup}) \vdash^*_M (q, \triangleright \underline{\sqcup} w) \}
        $$

        这个语言就是图灵可枚举的。接下来，我们将证明 R.E. 和图灵可枚举的等价性。

    -   **证明（$\Rightarrow$）**：现在，$L$ 是可以被 $M$ 可识别的（R.E.）。我们想让图灵机 $M'$ 列出所有 $L$ 里的字符串。

        这里有一个矛盾，$M'$ 如果遇到一次死循环（某个$w \notin L$），那我们就没办法列出后续可能的字符串了。为此，我们的解决办法是 **交替模拟**：

        -   第一步，模拟 $M(s_1)$ 运行 1 步；
        -   第二步，模拟 $M(s_1), M(s_2)$ 各运行 2 步；
        -   以此类推。

        这样，在任何一步中，如果发现 $M(s_i)$ 在限定步数内停机并接受了，则 $M'$ 切换到状态 $q$ 并把 $s_i$ 写在带子上（即“枚举”它）。我们通过这样的方式不重不漏地枚举。

    -   **证明（$\Leftarrow$）**：现在，存在一个枚举机（图灵机） $E$ 可以枚举 $L$，我们想让图灵机 $E'$ 识别 $L$。
        -   对于输入的串 $x$，启动枚举机 $E$。
        -   每当 $E$ 产出了一个字符串 $w$，$E'$ 就将 $w$ 与自己的输入 $x$ 比较。
        -   如果两者相等，$E'$ 停机。
        -   如果不相等，$E'$ 继续等待 $E$ 产生下一个串。
        -   这样，如果 $x \in L$，那迟早 $E'$ 会从 $E$ 那里匹配到一个；如果 $x \notin L$，那 $E'$ 就一直运行。这符合 R.E. 的规定。
...
!!! Warning "复杂度分析"
    标准 DTM 对 NTM 的模拟不同于模拟其他拓展图灵机那样一步一步模拟，而是指数级多的步数。所以，DTM 模拟 NTM 时时间复杂度的提升是**指数级**的，而别的是**多项式级别**的。
...
*   **极小化 (Minimalization)**：
    ...
    *   *注意*：这个搜索过程可能永远不停机（即对于某些输入 $x$，$y$ 不存在），此时产生的函数是**部分函数 (Partial Function)**（在部分输入上有定义）。
...
*   **证明思路**：
    *   ($\Rightarrow$)：图灵机可以模拟基本函数、合成、递归和极小化操作（通过循环搜索）。而 $\mu$-递归函数正是通过这几个操作构成的。
...
!!! Note "邱奇-图灵论题 (Church-Turing Thesis)"
    ...
    读者可以回忆一下什么是算法。我们要求算法必须能有一个输出。因此，算法对所有输入都会“停机”，因此一个算法永远是递归的（这也表明算法的范围小于 $\mu$-递归）。
    
    人类计算者 (Human Computer) 的计算过程（读写符号、状态转移、移动注意力）都可以被通用图灵机精确模拟。换句话说，一切直觉上可计算的函数（即人类可以用纸笔按某种机械步骤算出来的东西），都可以被一台图灵机计算。
...
!!! Question "这和接下来要了解的内容有什么关系？"
    ...
    -   我们知道，所有问题（语言）的集合是不可数的，然而图灵机的数量是可数的。这就必然导致有些问题无法被计算。
    -   CTT 认为，图灵机可以模拟任何计算过程，那它当然也可以模拟自己的代码。我们可以用这种自指能力来证明某问题不可判定，如停机问题。

### 6.1 UTM

我们想要做一个模拟图灵机的图灵机。由于图灵机的状态或者符号集没有上界，我们不可能直接把这种东西直接塞给这个模拟器。模拟器必须要有一个字母表。

我们知道，一个图灵机 $M = (K, \Sigma, \delta, s, H)$ 是一个有限的数学结构，那么它可以被编码成一个字符串：

-   对于符号集 $\Sigma \cup \{\leftarrow, \to\}$：
    -   令 $j = \min{j | j \in \mathbb Z \wedge 2^j \ge |\Sigma| + 2}$。
    -   这样，我们就可以用一个固定且有限长的字符串来编码每一个符号（课件上是 $a$ + $j$ 位的二进制串）。
    -   特别地，我们安排特殊符号 $\sqcup, \triangleright, \leftarrow, \to$ 固定为字典序最小的四个符号。
-   对于状态：
    -   令 $i = \min{i | i \in \mathbb Z \wedge 2^i \ge |K|}$。
    -   这样，我们就可以用一个固定且有限长的字符串来编码每一个状态（课件上是 $q$ + $i$ 位的二进制串）。
    -   特别地，我们用 $q0^i$ 表示起始状态。
-   对于图灵机本身（转移规则）：
    -   每个动作可以被拆解为一个四元组 $(q, a, p, b)$，代表 $q$ 状态下读到 $a$，状态转移至 $p$ 并执行 $b$ 操作。
    -   为了确保编码的唯一性，我们要对这些规则进行排序（按照字典序）。
    -   特别地，必须从编码列表必须从起始状态 $s$ 扫描到空格 $\sqcup$ 的那条规则 $\delta(s, \sqcup)$ 开始。

经过这样的编码，我们可以把它塞给这个模拟器了。这个模拟器就是 **通用图灵机**。

*   输入：$\langle M, w \rangle$。
*   功能：$U$ 模拟 $M$ 在 $w$ 上的每一步操作。一个想法是，先构建一个 3 带图灵机 $U'$，$U$ 作为标准图灵机来模拟 $U'$。

??? Note "三带图灵机的工作方式"
    *   **输入“$M$”“$w$”**：将“$M$”（图灵机的编码）复制到第二条磁带上，将输入字符串“$w$”置于第一条磁带的左端对齐，并在第三条磁带上写入起始状态“$s$”。
    *   **模拟 $M$ 的单步计算**：在第二条磁带中查找与 $M$ 当前状态及所扫描符号相对应的四元组（转移规则），并据此相应地更新第一条磁带（模拟 $M$ 的带子内容和磁头位置）及第三条磁带（更新 $M$ 的当前状态）。
    *   **重复步骤 (2)**：持续运行，直到第三条磁带的内容变为停机状态“$h$”（注：这一情况可能永远不会发生，即 $M$ 在 $w$ 上可能永不停机）。
    这就是三带通用图灵机的经典工作模式：
    1.  **第一条带（数据带）**：存储原始输入 $w$，模拟被执行机器 $M$ 的工作带。
    2.  **第二条带（程序带）**：存储 $M$ 的四元组列表（即 $M$ 的“源代码”）。
    3.  **第三条带（状态带）**：存储 $M$ 当前所处的内部状态，初始为 $s$，结束为 $h$。

*   结果：如果 $M$ 接受 $w$，$U$ 也接受；如果 $M$ 拒绝，$U$ 也拒绝（$U("M""w") = "M(w)"$）。如果 $M$ 死循环，$U$ 也死循环。
*   **结论**：语言 $A_{TM} = \{ \langle M, w \rangle \mid M \text{ accepts } w \}$ 是**递归可枚举 (R.E.)** 的。即我们能“识别”它，但不能“判定”它（因为死循环无法检测）。

UTM 是现代计算机“存储程序”思想的起源。

### 6.2.1 不可判定问题

这是本章最核心的内容，通过对角线法和归约法证明某些问题无解。

#### 停机问题
*   **问题定义**：$H = \{ \langle M, w \rangle \mid \text{TM} M \text{ halts on input } w \}$。
*   **对角线法（反证法）证明**：
    *   假设存在一个判定机 $R$ 能判定 $H$。即 $R(\langle M, w \rangle)$ 总能在有限步内回答 "Halt" 或 "Loop"。
    *   构造一个新的机器 $D$（对角线机器）：
        *   输入：$\langle M \rangle$。
        *   操作：调用 $R(\langle M, \langle M \rangle \rangle)$。
        *   行为：如果 $R$ 说 $M$ 会停机，则 $D$ 进入死循环；如果 $R$ 说 $M$ 死循环，则 $D$ 停机。
            
            diagonal($X$):
            
            $a$:    if $R(\langle X, \langle X \rangle \rangle)$ then goto $a$ else halt

    *   **矛盾**：我们考虑运行 $D(\langle D \rangle)$。
        *   若 $D$ 停机 $\implies R$ 说 $D$ 死循环 $\implies D$ 应该死循环。
        *   若 $D$ 死循环 $\implies R$ 说 $D$ 停机 $\implies D$ 应该停机。
    *   **结论**：导出矛盾，假设不成立。所以 $H$ 是不可判定的。
*   由此，$H$ 是 **R.E. 但不递归** 的。因为这个 $M$ 要么能停机，要么死循环（我们确实没办法明确判定能不能停机），因此满足半判定的含义。

!!! Success "怎么证明 $H$ 不是递归的？"
    仍然使用反证法。如果 $H$ 是递归的，那么这个语言（问题）也一定是递归的：
    
    $$
        H_1 = \{"M": M \text{ halts on input } "M"\}
    $$
    
    **递归语言对补运算封闭**。因此，$\bar{H_1} = \{"M": M \text{ does not halts on input } "M"\}$ 也一定是递归的。
    但是！$\bar{H_1}$ 甚至不是 R.E. 语言，怎么可能是递归的呢？

### 6.2.2 归约法 (Reduction)

这是证明新问题不可判定的标准工具。

*   **定义**：$A \le_m B$（$A$ 映射归约到 $B$）：如果存在可计算函数（递归函数） $f$，使得 $w \in A \iff f(w) \in B$。

    课件上用的表述更高级：A reduction from $L_1$ to $L_2$ is a recursive function $\tau: \Sigma* \to \Sigma$ such that $x \in L_1 \iff \tau(x) \in L_2$.
...
*   **常见归约路径**：
    ...
    *   $H$ $\le_m$ $REGULAR_{TM}$ (判定TM语言是否正则)
    *   $H$ $\le_m$ $TOTAL_{TM}$ (判定TM是否对所有输入停机)

### 6.2.3 其他不可判定问题

关于图灵机的以下问题都是不可判定 (Undecidable) 的。

*   **a) 通用停机问题（之前的 $H$）**：给定一个程序 $M$ 和输入 $w$，$M$ 在 $w$ 上会停机吗？
    *   *意义*：这是最基础的停机问题。
*   **b) 空带停机问题**：给定 $M$，$M$ 在空磁带（即输入为空串）上会停机吗？
    *   *意义*：即使限制了输入，只要程序足够复杂，依然算不出来。
*   **c) 存在性问题**：是否存在**任何**一个字符串能让 $M$ 停机？（即 $L(M)$ 是否为空集？）
    *   *意义*：你无法写一个编译器插件来检测“这段代码是否永远不会被执行到”。
*   **d) 完备性/全函数性问题**：$M$ 是否对**每一个**可能的输入字符串都能停机？
    *   *意义*：你无法写一个程序来完美验证另一个程序是否永远不会死循环。
*   **e) 等价性问题**：给定两个程序 $M_1$ 和 $M_2$，它们在相同的输入下表现一致吗（即识别的语言是否相同）？
    *   *意义*：这是软件工程的噩梦——你无法通过算法完美证明两段逻辑不同的代码在功能上是完全等价的。
*   **f) 语言性质判定**：$M$ 所识别的语言是正则语言吗？是上下文无关的吗？是递归的吗？
    *   *意义*：这引出了著名的**莱斯定理（Rice's Theorem）**：任何关于图灵机所识别语言的非平凡（Non-trivial）性质都是不可判定的。
*   **g) 特定机器的停机问题**：存在某个特定的图灵机 $M$（比如通用图灵机），对于它来说，仅判断输入 $w$ 是否能让它停机这一问题本身就是不可判定的。

数学上的证明套路是**规约**：
如果我们能写出一个算法解决 (b) 到 (f) 中的任何一个，我们就能利用这个算法作为“零件”去解决 (a) 停机问题。既然 (a) 已经被证明是不可解的，那么这些派生出来的问题必然也无解。

### 6.4 莱斯定理 (Rice's Theorem)
...
*   **应用**：判断 $L(M)$ 是否为空？是否有限？是否是正则语言？是否包含 "101"？由莱斯定理可知，这些统统不可判定。

### 6.5 其他系统的不可判定性
*   **文法问题**：对于给定的无限制文法 $G$ 和字符串 $w$，“判断 $w \in L(G)$”是不可判定的
*   **文法问题**：给定两个 CFG $G_1, G_2$，“判断 $L(G_1) \cap L(G_2) = \emptyset$”是不可判定的（而在 DFA 中是可判定的）。判断 CFG 是否有二义性也是不可判定的。
*   **铺砖问题 (Tiling)**：“给定一组骨牌，能否铺满整个平面？”不可判定。

!!! Note "等一下，有点晕。这里是不是有好多等价的表述？"
    
    -   任意图灵机变体在**计算能力**上等价——都等价于标准单带图灵机。
    -   对于 R.E. Languages，以下表述等价：
        1.  语言 $L$ 是 **递归可枚举的 (R.E.)**。
        2.  语言 $L$ 被某个图灵机 **半判定 (Semidecided)**（即：$w \in L \iff M$ 停机；$w \notin L \iff M$ 拒绝或死循环）。
        3.  语言 $L$ 被某个图灵机 **接受 (Accepted/Recognized)**。
        4.  语言 $L$ 被某个图灵机 **枚举 (Turing-enumerable)**。
        5.  语言 $L$ 由某个 **无限制文法 (Unrestricted Grammar)** 生成。
    -   对于递归语言，以下表述等价：
        1.  语言 $L$ 是 **递归的 (Recursive)**。
        2.  语言 $L$ 被某个图灵机 **判定 (Decided)**（即：$w \in L \implies M$ 接受；$w \notin L \implies M$ 拒绝；$M$ 永不死循环）。
        3.  语言 $L$ 是 R.E. 的，**且**其补集 $\bar{L}$ 也是 R.E. 的。
        4.  语言 $L$ 被某个图灵机 **按字典序枚举 (Lexicographically Turing-enumerable)**。
    -   以下三种定义描述了同一个函数集合：
        1.  图灵机可计算函数
        2.  $\mu$-递归函数
        3.  文法可计算函数

    | 核心概念 | 自动机模型 | 生成系统 (文法) | 函数系统 |
    | :---: | :---: | :---: | :---: |
    | **可计算 / R.E.** | 图灵机 (TM) / NTM | 无限制文法 (Type-0) | $\mu$-递归函数 |
    | **算法 / 递归** | 总是停机的 TM | / | / |
...
1.  **计算模型层级**：
    *   **DFA/NFA** (正则语言) $\subset$ **PDA** (上下文无关语言) $\subset$ **图灵机** (R.E. 语言)。
2.  **核心结论**：
    *   图灵机是计算能力的上限（邱奇-图灵论题）。
    *   有些问题是图灵机解决不了的（不可判定性）。
    *   判定问题的核心是**停机问题**，证明工具是**对角线法**和**归约**。
3.  **不同视角的统一**：
    *   **机器视角**：图灵机、枚举器。
    *   **语言视角**：递归语言、R.E. 语言。
    *   **函数视角**：$\mu$-递归函数。
