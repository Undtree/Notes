...
=\int_{-\infty}^{+\infty}f(x,z-x)\,\mathrm dx.
$$
...
超几何分布中，$\frac{N-n}{N-1}$ 是与不放回抽样有关的有限总体修正因子；若抽遍总体，$n=N$，则 $X=M$ 为常数、方差为 $0$。$N=1$ 时应直接按退化分布处理，不能代入含 $N-1$ 分母的公式。
...
若 $X,Y$ 不相关，则和与差的方差均为 $D(X)+D(Y)$。注意：即使是差，两个方差项也仍然相加。
